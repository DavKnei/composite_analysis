...
# --- Function to get MCS initiation times (No changes needed here) ---
...
def main_angle_calculation():
    parser = argparse.ArgumentParser(description="Calculate MCS movement angles for all tracks, then filter and save files for each region in a YAML file.")
    parser.add_argument("--input_csv", default="./csv/mcs_exp_GARextended_index.csv", help="Input CSV file with raw MCS track data.")
    ...
    # ---
    # ADDED: Filter to remove single-point tracks before any processing.
    # ---
    ...
    if not single_point_tracks.empty:
        ...
        # Optional: To see which tracks are being dropped, uncomment the line below
        # print(f"  Dropped track IDs: {single_point_tracks.tolist()}")
    ...
    for track_id, group_df in grouped_tracks:
        ...
        if L >= 2: # This condition is now always true, but kept for safety
            for i in range(L):
            ...
            # Post-processing logic for zero movement is kept as per user request
    ...
    # Merge all calculated angles back into the main DataFrame
    ...
    # 3. Post-calculation: Subdivide into regions and save files
    print("\n--- Subdividing data into defined regions ---")
    try:
    ...
    for region_name, region_bounds in regions_data.items():
        ...
        # Filter the master dataframe (with angles) for the current region
        df_region = df_full[
            (df_full['center_lon'] >= region_bounds['lon_min']) &
            (df_full['center_lon'] <= region_bounds['lon_max']) &
            (df_full['center_lat'] >= region_bounds['lat_min']) &
            (df_full['center_lat'] <= region_bounds['lat_max'])
        ].copy()

        if df_region.empty:
            print(f"No MCS track points found within region '{region_name}'. Skipping.")
            continue
        ...
        # Save the filtered data (all steps) for the region
        Path(output_csv_all_steps_path).parent.mkdir(parents=True, exist_ok=True)
        df_region.to_csv(output_csv_all_steps_path, index=False, float_format='%.3f')
        print(f"  Saved {len(df_region)} track points to {Path(output_csv_all_steps_path).name}")

        # Extract and save initiation data for the region
        extract_mcs_initiation_data(output_csv_all_steps_path, output_csv_initiation_path)

    print("\nScript finished successfully.")

if __name__ == "__main__":
    main_angle_calculation()