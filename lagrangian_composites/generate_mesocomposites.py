...
def rotate_centered_grid(
    ds_centered_source, event_motion_angle_deg, target_x_km_coords, target_y_km_coords
):
    """
    Rotates an already centered dataset according to the event's motion.
    (This function is correct and remains unchanged)
    """
    current_alignment_angle_deg = (
        event_motion_angle_deg if not pd.isna(event_motion_angle_deg) else 0.0
    )

    ds_centered_source = ds_centered_source.rename(
        {"x_relative_km": "x_interm", "y_relative_km": "y_interm"}
    )
    ...
    x_sample_points = target_xx_m * math.cos(alpha_rad) + target_yy_m * math.sin(
        alpha_rad
    )
    y_sample_points = -target_xx_m * math.sin(alpha_rad) + target_yy_m * math.cos(
        alpha_rad
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate mesoscale composites from pre-centered event data."
    )
    parser.add_argument(
        "--csv_path",
        default="./csv/event_centered_composite.csv",
        help="Path to the MCS events CSV file containing angles of movement.",
    )
    parser.add_argument(
        "--meso", action="store_true", help="Use mesoscale-separated data as input."
    )
    parser.add_argument(
        "--output_dir",
        default="/home/dkn/mesocomposites/ERA5/",
        help="Directory to save the final composite NetCDF files.",
    )
    args = parser.parse_args()

    BASE_DIR = Path("/home/dkn/mesocomposites/ERA5/")
    # Dynamically determine output filename from the input CSV
    output_dir = Path(args.output_dir)
    csv_path = Path(args.csv_path)

    if args.meso:
        logging.info("--- Running in MESOSCALE mode ---")
        input_dir = BASE_DIR / "temp_files/meso"
        rotated_temp_dir = BASE_DIR / "temp_files_rotated/meso"
        output_composite_file = output_dir / f"{csv_path.stem}_mesoscale.nc"
    else:
        logging.info("--- Running in SYNOPTIC mode ---")
        input_dir = BASE_DIR / "temp_files/synoptic"
        rotated_temp_dir = BASE_DIR / "temp_files_rotated/synoptic"
        output_composite_file = output_dir / f"{csv_path.stem}_synoptic.nc"
    ...
    try:
        ...
        print(
            f"ERROR: Could not load or parse the required CSV file. Please check the path and format. Details: {e}"
        )
        return
    ...
    pbar = tqdm(
        events_df.itertuples(), desc="Checking & Rotating Events", total=len(events_df)
    )
    for event in pbar:
        ...
        try:
            ...
            event_angle_degrees = events_with_index.loc[
                track_number, "angle_of_movement"
            ]
    ...
    if not files_for_composite:
        print(
            "No events were successfully found or processed. No output file will be created."
        )
        return

    print(f"Aggregating {len(files_for_composite)} successfully rotated events...")
    variable_list = {}
    for file in files_for_composite:
        ds = xr.open_dataset(file)
        variable_list[str(ds["track_number"].values[0])] = list(ds.data_vars)

    required_variables = ["t", "q", "z", "u", "v", "w"]
    keys_with_missing_data = []
    for key, variables in variable_list.items():
        if not all(var in variables for var in required_variables):
            keys_with_missing_data.append(key)

    print(f"Keys with missing data variables: {keys_with_missing_data}")

    combined_ds = xr.open_mfdataset(
        files_for_composite, concat_dim="track_number", combine="nested"
