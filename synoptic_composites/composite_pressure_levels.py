...
# Includes base variables and now also theta_e for its "true" climatology.
...
def process_month_level_events(task: Tuple) -> Dict[str, Any]:
    ...
    if clim_ds_input is not None:
        if not {'month', 'hour', 'level'}.issubset(clim_ds_input.coords):
            raise ValueError(f"Task {task_label}: Climatology dataset missing required coordinates (month, hour, level).")
        ...
        if month in TARGET_MONTHS:
            ...
            logging.debug(f"Task {task_label}: Month {month} is not a JJA month. Clim sum will be None for this task's month.")
    else:
    ...
    with xr.open_dataset(file_path, chunks={'time': 'auto'}, cache=False) as ds:
        ...
        for lev_val in levels:
            ...
            if not base_vars_present_in_raw:
                logging.warning(f"Task {task_label} L{lev_val}: No base variables found in raw data for count.")
                comp_data[level_key]['count'] = None
                continue

            count_arr = ds_level_raw_data[base_vars_present_in_raw].to_array(dim="variable_for_count").notnull().any(dim="variable_for_count").sum(dim='time').compute()
            ...
            logging.debug(f"Task {task_label} L{lev_val}: count_arr shape: {count_arr.shape}, max: {np.max(count_arr) if count_arr.size>0 else 'N/A'}")
            ...
            for var in VAR_LIST:
                ...
                if var in ds_level_raw_data:
                    ...
                    if comp_data[level_key]['lat'] is not None:
                        shape_sum = (len(comp_data[level_key]['lat']), len(comp_data[level_key]['lon']))
                        comp_data[level_key][f"{var}_sum"] = np.full(shape_sum, np.nan, dtype=np.float32)
                
                # Climatology sum for VAR_LIST variables
                if clim_level_month_aligned is not None and var in clim_level_month_aligned:
                    ...
                    clim_data_for_events_np = clim_level_month_aligned[var].sel(hour=xr.DataArray(event_hours, dims="event_hours_dim")).compute().data
                    clim_sum_arr = np.nansum(clim_data_for_events_np, axis=0)
                    comp_data[level_key][f"{var}_clim_sum"] = clim_sum_arr
                else:
                    logging.debug(f"Task {task_label} L{lev_val} V={var}: Skipping climatology sum (clim not aligned or var missing in clim).")
                    if comp_data[level_key]['lat'] is not None:
                        shape_sum = (len(comp_data[level_key]['lat']), len(comp_data[level_key]['lon']))
                        comp_data[level_key][f"{var}_clim_sum"] = np.full(shape_sum, np.nan, dtype=np.float32)
                
                logging.debug(f"Task {task_label} L{lev_val} V={var}: Sum shape: {var_sum_arr.shape if var_sum_arr is not None else 'None'}, NaNs: {np.isnan(var_sum_arr).sum() if var_sum_arr is not None else 'N/A'}")
            ...
            if 't' in ds_level_raw_data and 'q' in ds_level_raw_data and ds_level_raw_data.time.size > 0:
                ...
                if comp_data[level_key]['lat'] is not None:
                    shape_sum = (len(comp_data[level_key]['lat']), len(comp_data[level_key]['lon']))
                    comp_data[level_key]["theta_e_sum"] = np.full(shape_sum, np.nan, dtype=np.float32)
            
            # CLIMATOLOGY THETA_E (loaded from pre-calculated climatology file)
            if clim_level_month_aligned is not None and 'theta_e' in clim_level_month_aligned and \
               clim_level_month_aligned.hour.size > 0 and len(event_hours) > 0:
                ...
                logging.debug(f"Task {task_label} L{lev_val}: Skipping loaded theta_e_clim_sum (theta_e missing in clim, zero hours, or no event_hours).")
                if comp_data[level_key]['lat'] is not None:
                    shape_sum = (len(comp_data[level_key]['lat']), len(comp_data[level_key]['lon']))
                    comp_data[level_key]["theta_e_clim_sum"] = np.full(shape_sum, np.nan, dtype=np.float32)
    ...
    for lev_val_chk in levels: 
        ...
        if level_key_chk in comp_data and comp_data[level_key_chk].get('count') is not None:
             ...
             logging.debug(f"  Final state L{lev_val_chk}: Count max = {np.max(count_final) if count_final.size > 0 else 'N/A'}")
             for var_chk in ALL_VARS_TO_SAVE:
                 sum_is_none = comp_data[level_key_chk].get(f'{var_chk}_sum') is None
                 clim_sum_is_none = comp_data[level_key_chk].get(f'{var_chk}_clim_sum') is None
                 logging.debug(f"    Final state L{lev_val_chk} V={var_chk}: sum is None? {sum_is_none}, clim_sum is None? {clim_sum_is_none}")
        else:
...
def main():
    ...
    for wt_val in weather_types_to_process: 
        ...
        logging.info(f"Processing WT {wt_val} ('{'All Events' if wt_val==0 else f'Type {wt_val}'}') - {len(df_wt)} events (JJA, Period: {args.period})")
        ...
        for off_val in time_offsets_parsed: 
            ...
            if offset_col not in df_wt.columns: 
                 logging.error(f"Offset column '{offset_col}' missing for WT {wt_val}, offset {off_val}h. This should not happen.")
                 continue
            ...
            logging.info(f"    Finished processing tasks for WT {wt_val}, Offset {off_val}h.")

            tasks_by_month_comp = {}
            for task_data, res in zip(tasks, monthly_tasks_results):
                _, month_val_task, _, _, _, _ = task_data 
