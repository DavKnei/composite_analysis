...
def main():
    ...
    comp_path_multi = os.path.join(args.base_dir, args.comp_file_multidim + f"{args.region}_wt_clim_{args.period}.nc")
    ...
    available_wts_data = ds_multi.weather_type.data
    wts_to_plot = [wt for wt in wts_to_plot if wt in available_wts_data]

    for weather_type in wts_to_plot:
        ds_multi_sel = ds_multi.sel(weather_type=weather_type)
        ds_multi_sel = ds_multi_sel.sel(month=months_list).mean(dim="month")
        ...
        event_count_months_arr = ds_multi["event_count"].sel(time_diff=0, weather_type=weather_type, month=months_list, level=args.pressure_level).data
        current_sum = 0
        if event_count_months_arr.ndim == 3 and event_count_months_arr.shape[0] == len(months_list):
            ...
            n_for_suptitle = int(round(current_sum))
        else:
            n_for_suptitle = np.nan

        ds_precip_sel = ds_precip.sel(weather_type=weather_type, month=months_list).mean(dim="month")
        ...
        weather_type_str = lamb_weather_type_label(int(weather_type))
        plt.suptitle(
            f"{args.region.replace('_',' ').title()} ({month_label})\n"
            f"Weather Type: {weather_type_str} ({n_for_suptitle:.0f} events)",
            fontsize=16
        )
        # Reduce white space between subplots.
        plt.subplots_adjust(top=0.95, bottom=0.005, left=0.03, right=0.97, hspace=0.0, wspace=0.05)
        out_file = args.output_prefix + args.region + '_' + month_label + f'_wt{weather_type}.png'
