...
specified region during a surrounding time window (e.g., +/- 24 hours).
...
Example:
  python 04_get_nomcs_times.py --input ./csv/mcs_exp_GAR_index.csv \
                               --region eastern_alps \
                               --output ./synoptic_composites/csv/composite_eastern_alps_nomcs.csv \
...
# import os # Uncomment if using os.makedirs

# Define subregion boundaries (copied from 03_filter_composite_times.py)
SUBREGIONS = {
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="synoptic_composites/csv/mcs_exp_GAR_index.csv", # Default from 02_get_composite_dates.py
        help="Input CSV file with ALL MCS track timesteps (e.g., mcs_exp_GAR_index.csv)"
    )
    parser.add_argument(
        "--region",
        type=str,
        required=True,
        help=f"Subregion to consider (e.g., {', '.join(SUBREGIONS.keys())})"
    )
    parser.add_argument(
        "--weather_type_csv",
        type=str,
    ...
    if region_key not in SUBREGIONS:
        ...
        sys.exit(1) # Exit if region is invalid
    ...
    mcs_region_times = pd.to_datetime(mcs_region_times) # Ensure they are datetime objects

    # --- Generate Full Timeline ---
    min_time = mcs_df_full['datetime'].min()
    max_time = mcs_df_full['datetime'].max()
    if pd.isna(min_time) or pd.isna(max_time):
        print("Error: Could not determine time range from MCS data (min/max is NaN). Check input file.")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error: Invalid frequency string '{args.freq}'. Please use a valid pandas offset alias (e.g., '30min', 'H', 'D'). Error: {e}")
        sys.exit(1)


    if full_timeline.empty:
        print(f"Error: Generated timeline is empty. Check frequency ('{args.freq}') and date range ({min_time} to {max_time}).")
        sys.exit(1)
    ...
    if len(mcs_region_times) > 0: # Only need to mark if there were MCS times in the region
        # Using searchsorted for potentially better performance than iteration over is_nomcs index
        timeline_numeric = full_timeline.values.astype(np.int64) # Convert timeline to numeric for faster search
        ...
        for mcs_time in mcs_region_times:
            ...
            end_idx = np.searchsorted(timeline_numeric, end_window_ns, side='right') # Use 'right' to include end time if it matches exactly
        ...
        print(f"Marked {affected_count} unique time steps as MCS-affected (including windows).")
    else:
        print("No MCS times found in the region, so all timeline steps are considered potential 'noMCS'.")
    ...
    if nomcs_times.empty:
        ...
        nomcs_df = pd.DataFrame({'datetime': pd.Series(dtype='datetime64[ns]'), 'wt': pd.Series(dtype='object')})
    else:
    ...
    try:
        ...
        if 'wt' in wt_df.columns:
            wt_col_name_found = 'wt'
             ...
             print(f"Warning: Could not find standard weather type column ('wt' or 'weather_type') in {weather_csv}.")
        ...
        if wt_col_name_found:
            ...
            if wt_col_name_found != 'wt':
                 wt_df_filtered = wt_df_filtered.rename(columns={wt_col_name_found: 'wt'})
            ...
            if not nomcs_df.empty: # Only merge if there are noMCS times
                 ...
                 if missing_wt > 0:
                      print(f"Warning: {missing_wt} out of {len(nomcs_df)} 'noMCS' times could not be matched with a weather type in {weather_csv}.")
                 ...
                 print(f"Weather type file {weather_csv} loaded, but no 'noMCS' times to merge with.")
             ...
             print(f"No weather type column found in {weather_csv}, 'wt' column filled with NaN.")


    except FileNotFoundError:
        print(f"Warning: Weather types file not found at {weather_csv}. 'wt' column will be NaN.")
        ...
        print(f"Warning: Could not load or merge weather types from {weather_csv} ({e}). 'wt' column will be NaN.")
